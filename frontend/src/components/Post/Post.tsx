...
const Post = ({ author_name, author_pic, id, post_url, posted_date, text, comments, images, likes_count }: PostType) => {
    ...
    const postLikeHandler = (toSet: boolean) => {
        setUserLiked(toSet);
    }
        ...
        <div className="flex w-full max-w-3xl flex-col sm:flex-row">
            <div className="w-10 h-10 relative rounded-full bg-gray-50/[0.5] overflow-hidden">
                <Link href={`/profile/${author_name}`}>
                    <a>
                        <Image src={author_pic} layout="fill" objectFit="cover" alt={author_name} />
                    </a>
                </Link>
            </div>
            <div className="flex-1 mt-1 sm:mt-0 sm:mx-4">
                ...
                <div className="my-3 py-3 border-y flex gap-4 text-sm">
                    {/* like */}
                    <button className="flex items-center" onClick={() => postLikeHandler(!userLiked)}>
                        {/* icon */}
                        <span className={`relative ${userLiked ? "text-secondary-100" : ""}`} style={{ top: "-2px" }}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                        ...
                        <span className={`mx-2 ${userLiked ? "text-secondary-100" : ""}`}>Like</span>
                        {/* count */}
                        <span className={`px-2 text-xs ${userLiked ? "text-secondary-100 bg-secondary-100/[0.1]" : "text-gray-500 bg-gray-100"} rounded-full`}>{likes_count}</span>
                    </button>
                    {/* comment */}
